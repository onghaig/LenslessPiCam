...
sensor_dict = {
    ...
    # Arducam IMX708 Camera Sensor
    # Datasheet: 
    # IMX708
    SensorOptions.ARDU_708.value: {
        SensorParam.PIXEL_SIZE: np.array([1.4e-6, 1.4e-6]),       # Pixel size: 1.4 µm × 1.4 µm
        SensorParam.RESOLUTION: np.array([4608, 2592]),           # Resolution: 4608 × 2592 (≈11.9 MP)
        SensorParam.DIAGONAL: 7.4e-3,                             # Sensor diagonal ≈7.4 mm (1/2.43")
        SensorParam.COLOR: True,                                  # Color sensor
        SensorParam.BIT_DEPTH: [10,12],                          # RAW10 output, 12-bit internal capture
        SensorParam.MAX_EXPOSURE: 112,                           # Maximum exposure time not specified
        SensorParam.MIN_EXPOSURE: 1e-4,                           # Minimum exposure time not specified
    },
